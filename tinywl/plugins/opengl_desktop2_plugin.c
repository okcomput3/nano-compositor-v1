...
static GLuint compile_shader(GLenum type, const char* source) {
    ...
    if (!compiled) {
        ...
        if (length > 0) {
            ...
            loggy_opengl_desktop2( "Shader error: %s\n", log);
            free(log);
        }
...
static bool init_headless_renderer(struct plugin_state *state) {
    ...
    if (!state->renderer) {
        loggy_opengl_desktop2( "[PLUGIN] Failed to create wlr_gles2_renderer\n");
        close(drm_fd);
        return false;
    }
    ...
    if (!state->allocator) {
        loggy_opengl_desktop2( "[PLUGIN] Failed to create wlr_allocator\n");
        wlr_renderer_destroy(state->renderer);
        return false;
    }

    loggy_opengl_desktop2( "[PLUGIN] ✅ Successfully initialized headless wlroots context\n");
    return true;
}
static int create_dmabuf(size_t size, int instance_id) {
    ...
    if (lock_fd < 0) {
        loggy_opengl_desktop2( "[PLUGIN-%d] Failed to create lock file: %s\n", instance_id, strerror(errno));
        return -1;
    }
    
    // Lock for multi-process safety
    if (flock(lock_fd, LOCK_EX) < 0) {
        loggy_opengl_desktop2( "[PLUGIN] PID=%d Failed to acquire lock: %s\n", pid, strerror(errno));
        close(lock_fd);
        return -1;
    }
    
    loggy_opengl_desktop2( "[PLUGIN] PID=%d Creating DMA-BUF of size %zu bytes\n", 
        pid, size);
    
    // Method 1: Try DMA heap allocation
    const char* heap_path = "/dev/dma_heap/system";
    loggy_opengl_desktop2( "[PLUGIN] PID=%d Attempting to open: %s\n", pid, heap_path);
    
    int heap_fd = open(heap_path, O_RDONLY | O_CLOEXEC);
    if (heap_fd < 0) {
        loggy_opengl_desktop2( "[PLUGIN] PID=%d Failed to open %s: %s (errno=%d)\n", 
                pid, heap_path, strerror(errno), errno);
        goto try_fallback;
    }
    
    loggy_opengl_desktop2( "[PLUGIN] PID=%d ✅ Successfully opened %s (fd=%d)\n", 
    ...
    if (ioctl_result == 0) {
        loggy_opengl_desktop2( "[PLUGIN] PID=%d ✅ DMA heap allocation SUCCESS: fd=%d\n", 
                pid, heap_data.fd);
        
        // Test the returned fd
        struct stat stat_buf;
        if (fstat(heap_data.fd, &stat_buf) == 0) {
            loggy_opengl_desktop2( "[PLUGIN] PID=%d DMA-BUF fd stats: size=%ld, mode=0%o\n", 
    ...
    loggy_opengl_desktop2( "[PLUGIN] PID=%d ioctl DMA_HEAP_IOCTL_ALLOC FAILED: %s (errno=%d)\n", 
            pid, strerror(errno), errno);
    
    // Method 2: Try smaller allocation if the original was large
    if (size > 1024 * 1024) {  // If > 1MB
        loggy_opengl_desktop2( "[PLUGIN] PID=%d Large allocation failed, trying smaller test allocation...\n", pid);
        ...
        if (ioctl(heap_fd, DMA_HEAP_IOCTL_ALLOC, &test_data) == 0) {
            loggy_opengl_desktop2( "[PLUGIN] PID=%d ✅ Small test allocation worked! The issue might be size limits.\n", pid);
            ...
            if (ioctl(heap_fd, DMA_HEAP_IOCTL_ALLOC, &medium_data) == 0) {
                loggy_opengl_desktop2( "[PLUGIN] PID=%d ✅ Medium allocation worked: fd=%d\n", 
                        pid, medium_data.fd);
                result = medium_data.fd;
                goto cleanup_heap_fd;
            } else {
                loggy_opengl_desktop2( "[PLUGIN] PID=%d Medium allocation failed: %s\n", 
                        pid, strerror(errno));
            }
        } else {
            loggy_opengl_desktop2( "[PLUGIN] PID=%d Even small test allocation failed: %s\n", 
                    pid, strerror(errno));
        }
    }
...
try_fallback:
    ...
    loggy_opengl_desktop2( "[PLUGIN] PID=%d DMA heap failed, falling back to memfd (won't work for zero-copy)\n", pid);
    
    int memfd = memfd_create("plugin-fbo-fallback", MFD_CLOEXEC);
    if (memfd < 0) {
        loggy_opengl_desktop2( "[PLUGIN] PID=%d memfd_create failed: %s\n", pid, strerror(errno));
        goto error;
    }
    
    if (ftruncate(memfd, size) != 0) {
        loggy_opengl_desktop2( "[PLUGIN] PID=%d memfd ftruncate failed: %s\n", pid, strerror(errno));
        close(memfd);
        goto error;
    }
    
    loggy_opengl_desktop2( "[PLUGIN] PID=%d ⚠️  Created memfd fallback: fd=%d (THIS WON'T WORK FOR ZERO-COPY)\n", 
            pid, memfd);
    result = memfd;
    goto success;

error:
    loggy_opengl_desktop2( "[PLUGIN] PID=%d ❌ All allocation methods failed\n", pid);
    result = -1;
...
static void close_dmabuf(int fd) {
    if (fd >= 0) {
        pid_t pid = getpid();
        loggy_opengl_desktop2( "[PLUGIN] PID=%d Closing dmabuf fd=%d\n", pid, fd);
        close(fd);
    }
}
...
static bool init_dmabuf_fbo_robust(struct plugin_state *state, int width, int height) {
    loggy_opengl_desktop2( "[PLUGIN] Attempting robust DMA-BUF backed FBO (%dx%d)\n", width, height);
    ...
    for (int attempt = 0; attempt < 3; attempt++) {
        ...
        loggy_opengl_desktop2( "[PLUGIN] Attempt %d: Created DMA-BUF fd: %d\n", attempt + 1, state->dmabuf_fd);
        ...
        if (is_real_dmabuf) {
            loggy_opengl_desktop2( "[PLUGIN] ✅ Confirmed real DMA-BUF, proceeding with EGL import\n");
        } else {
            // Instead of rejecting, let's check if it's a usable memfd
            struct stat fd_stat;
            if (fstat(state->dmabuf_fd, &fd_stat) == 0 && fd_stat.st_size == (width * height * 4)) {
                loggy_opengl_desktop2( "[PLUGIN] ⚠️  Not a real DMA-BUF but valid memfd, proceeding\n");
            } else {
                loggy_opengl_desktop2( "[PLUGIN] ❌ FD %d is invalid, trying next attempt\n", state->dmabuf_fd);
        ...
        if (!eglCreateImageKHR || !glEGLImageTargetTexture2DOES) {
            loggy_opengl_desktop2( "[PLUGIN] EGL extensions not available\n");
            close(state->dmabuf_fd);
            state->dmabuf_fd = -1;
            return false;
        }
        ...
        if (state->egl_image == EGL_NO_IMAGE_KHR) {
            ...
            loggy_opengl_desktop2( "[PLUGIN] EGL image creation failed (attempt %d): 0x%x\n", 
                    attempt + 1, error);
            close(state->dmabuf_fd);
            state->dmabuf_fd = -1;
            continue;
        ...
        if (gl_error != GL_NO_ERROR) {
            loggy_opengl_desktop2( "[PLUGIN] Failed to create texture from EGL image: 0x%x\n", gl_error);
            
            switch (gl_error) {
                case GL_INVALID_VALUE:
                    loggy_opengl_desktop2( "[PLUGIN] GL_INVALID_VALUE - Invalid texture target or EGL image\n");
                    break;
                case GL_INVALID_OPERATION:
                    loggy_opengl_desktop2( "[PLUGIN] GL_INVALID_OPERATION - Context/texture state issue\n");
                    break;
                default:
                    loggy_opengl_desktop2( "[PLUGIN] Unknown OpenGL error\n");
                    break;
            }
        ...
        loggy_opengl_desktop2( "[PLUGIN] ✅ Created GL texture %u from EGL image\n", state->fbo_texture);
        ...
        if (fb_status == GL_FRAMEBUFFER_COMPLETE) {
            ...
            if (is_real_dmabuf) {
                loggy_opengl_desktop2( "[PLUGIN] ✅ TRUE ZERO-COPY: Real DMA-BUF FBO created on attempt %d!\n", attempt + 1);
            } else {
                loggy_opengl_desktop2( "[PLUGIN] ✅ PSEUDO ZERO-COPY: MemFD FBO created on attempt %d!\n", attempt + 1);
            }
            return true;
        } else {
            loggy_opengl_desktop2( "[PLUGIN] Framebuffer incomplete on attempt %d: 0x%x\n", 
                    attempt + 1, fb_status);
    ...
    loggy_opengl_desktop2( "[PLUGIN] ❌ All DMA-BUF FBO creation attempts failed\n");
    return false;
}
/**
...
static bool init_plugin_egl(struct plugin_state *state) {
    ...
    if (!eglChooseConfig(state->egl_display, config_attribs, &config, 1, &config_count)) {
        loggy_opengl_desktop2( "[PLUGIN-%d] Failed to choose EGL config\n", state->instance_id);
        return false;
    }

    if (config_count == 0) {
        loggy_opengl_desktop2( "[PLUGIN-%d] No suitable EGL config found\n", state->instance_id);
        return false;
    }

    loggy_opengl_desktop2( "[PLUGIN-%d] Found %d EGL config(s)\n", state->instance_id, config_count);
    ...
    if (state->egl_context == EGL_NO_CONTEXT) {
        EGLint error = eglGetError();
        loggy_opengl_desktop2( "[PLUGIN-%d] Failed to create EGL context: 0x%x\n", state->instance_id, error);
        
        // Fallback: try simpler context creation
        loggy_opengl_desktop2( "[PLUGIN-%d] Trying fallback context creation...\n", state->instance_id);
        ...
        if (state->egl_context == EGL_NO_CONTEXT) {
            loggy_opengl_desktop2( "[PLUGIN-%d] Fallback context creation also failed\n", state->instance_id);
            return false;
        }
    }

    loggy_opengl_desktop2( "[PLUGIN-%d] ✅ EGL context created: %p\n", state->instance_id, state->egl_context);
    ...
    if (state->egl_surface == EGL_NO_SURFACE) {
        EGLint error = eglGetError();
        loggy_opengl_desktop2( "[PLUGIN-%d] Failed to create PBuffer surface: 0x%x\n", state->instance_id, error);
        eglDestroyContext(state->egl_display, state->egl_context);
        return false;
    }

    loggy_opengl_desktop2( "[PLUGIN-%d] ✅ PBuffer surface created: %p\n", state->instance_id, state->egl_surface);
    ...
    if (!eglMakeCurrent(state->egl_display, state->egl_surface, state->egl_surface, state->egl_context)) {
        EGLint error = eglGetError();
        loggy_opengl_desktop2( "[PLUGIN-%d] Failed to make EGL context current: 0x%x\n", state->instance_id, error);
    ...
    loggy_opengl_desktop2( "[PLUGIN-%d] ✅ OpenGL Context Active:\n", state->instance_id);
    loggy_opengl_desktop2( "[PLUGIN-%d]   Vendor: %s\n", state->instance_id, gl_vendor ? gl_vendor : "Unknown");
    loggy_opengl_desktop2( "[PLUGIN-%d]   Renderer: %s\n", state->instance_id, gl_renderer ? gl_renderer : "Unknown");
    loggy_opengl_desktop2( "[PLUGIN-%d]   Version: %s\n", state->instance_id, gl_version ? gl_version : "Unknown");
    ...
    GLenum gl_error = glGetError();
    if (gl_error != GL_NO_ERROR) {
        loggy_opengl_desktop2( "[PLUGIN-%d] OpenGL context test failed: 0x%x\n", state->instance_id, gl_error);
    ...
    loggy_opengl_desktop2( "[PLUGIN-%d] ✅ EGL context fully initialized and tested\n", state->instance_id);
    return true;
}
*/
...
static GLuint texture_from_dmabuf(struct plugin_state *state, const ipc_window_info_t *info) {
    if (!state->dmabuf_ctx.initialized) {
        loggy_opengl_desktop2( "[PLUGIN] Shared DMA-BUF context not available, using fallback\n");
        return 0;  // Will fallback to stride fix method
    }
    
    loggy_opengl_desktop2( "[PLUGIN] Importing window DMA-BUF via shared library: %dx%d, fd=%d, format=0x%x, modifier=0x%lx\n",
   ...
   // loggy_opengl_desktop2( "[PLUGIN] 🔍 Shared library done, closing FD %d\n", info->fd);
  //  close(info->fd);
    
    if (texture > 0) {
        loggy_opengl_desktop2( "[PLUGIN] ✅ Imported window DMA-BUF via shared library: texture %u\n", texture);
    } else {
        loggy_opengl_desktop2( "[PLUGIN] Shared library DMA-BUF import failed for window\n");
    }
...
static void check_fd_usage(void) {
    ...
    if (++check_counter % 60 == 0) {
        ...
        loggy_opengl_desktop2( "[PLUGIN] Open file descriptors: %d/1024\n", fd_count);
        
        if (fd_count > 900) {
            loggy_opengl_desktop2( "[PLUGIN] WARNING: High FD usage! Possible leak!\n");
        }
    }
}
...
static void send_frame_notification_smart(struct plugin_state *state) {
    ...
    if (should_notify) {
        ...
        if (++notification_counter <= 3) {
            loggy_opengl_desktop2("[PLUGIN] Sent notification #%d\n", notification_counter);
        }
    }
}
...
static int get_free_pool_buffer(struct plugin_state *state) {
    ...
    for (int i = 0; i < state->dmabuf_pool.pool_size; i++) {
        if (!state->dmabuf_pool.in_use[i]) {
            ...
            loggy_opengl_desktop2( "[PLUGIN] 📦 Using pool buffer %d (fd=%d)\n", i, state->dmabuf_pool.fds[i]);
            return i;
        }
    }
    
    loggy_opengl_desktop2( "[PLUGIN] ⚠️  No free pool buffers available\n");
...
static void release_pool_buffer(struct plugin_state *state, int buffer_id) {
    if (buffer_id >= 0 && buffer_id < state->dmabuf_pool.pool_size) {
        ...
        loggy_opengl_desktop2( "[PLUGIN] 📤 Released pool buffer %d\n", buffer_id);
    }
}
...
static void test_pool_rendering(struct plugin_state *state) {
    loggy_opengl_desktop2( "[PLUGIN] === STEP 3: Testing pool-based rendering ===\n");
    
    if (!state->dmabuf_pool.initialized) {
        loggy_opengl_desktop2( "[PLUGIN] ❌ Pool not initialized\n");
        return;
    }
...
static bool send_pool_to_compositor(struct plugin_state *state) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        loggy_opengl_desktop2( "[PLUGIN] 📤 Packing pool buffer %d: fd=%d -> dup_fd=%d\n", 
                i, state->dmabuf_pool.fds[i], fd_data[i]);
    }

    // Send the message with all FDs at once
    if (sendmsg(state->ipc_fd, &msg, 0) < 0) {
        loggy_opengl_desktop2( "[PLUGIN] ❌ Failed to send pool FDs: %s\n", strerror(errno));
    ...
    loggy_opengl_desktop2( "[PLUGIN] ✅ Sent pool (4 FDs) to compositor in single message!\n");
    return true;
}
static void test_pool_usage(struct plugin_state *state) {
    loggy_opengl_desktop2( "[PLUGIN] === STEP 2: Testing pool usage ===\n");
    ...
    loggy_opengl_desktop2( "[PLUGIN] Got buffers: %d, %d, %d\n", buf1, buf2, buf3);
    
    // Release one and get another
    release_pool_buffer(state, buf1);
    int buf4 = get_free_pool_buffer(state);
    
    loggy_opengl_desktop2( "[PLUGIN] After release/reuse: buf4=%d\n", buf4);
    ...
    loggy_opengl_desktop2( "[PLUGIN] ✅ Pool usage test complete\n");
}
...
static bool init_egl_extensions(EGLDisplay display) {
    ...
    if (!eglCreateImageKHR || !eglDestroyImageKHR) {
        loggy_opengl_desktop2( "[PLUGIN] Basic EGL image extensions not available\n");
        return false;
    }
    
    if (!eglExportDMABUFImageQueryMESA || !eglExportDMABUFImageMESA) {
        loggy_opengl_desktop2( "[PLUGIN] MESA DMA-BUF export extensions not available\n");
        return false;
    }
...
static int export_texture_as_dmabuf(struct plugin_state *state) {
    if (!state->fbo_texture || state->fbo_texture == 0) {
        loggy_opengl_desktop2( "[PLUGIN] No FBO texture to export (texture=%u)\n", state->fbo_texture);
        return -1;
    }
    
    // Check if extensions are available
    if (!eglCreateImageKHR) {
        loggy_opengl_desktop2( "[PLUGIN] eglCreateImageKHR not available\n");
        return -1;
    }
    
    // Make sure context is current
    if (!eglMakeCurrent(state->egl_display, state->egl_surface, state->egl_surface, state->egl_context)) {
        loggy_opengl_desktop2( "[PLUGIN] Failed to make EGL context current: 0x%x\n", eglGetError());
        return -1;
    }
    ...
    if (egl_image == EGL_NO_IMAGE_KHR) {
        loggy_opengl_desktop2( "[PLUGIN] Failed to create EGL image from FBO texture\n");
        return -1;
    }
    ...
    if (!eglExportDMABUFImageQueryMESA(state->egl_display, egl_image, 
        ...
        loggy_opengl_desktop2( "[PLUGIN] Failed to query DMA-BUF attributes\n");
    ...
    if (!eglExportDMABUFImageMESA(state->egl_display, egl_image,
        ...
        loggy_opengl_desktop2( "[PLUGIN] Failed to export DMA-BUF\n");
        eglDestroyImageKHR(state->egl_display, egl_image);
        return -1;
    }
    ...
    loggy_opengl_desktop2( "[PLUGIN] ✅ Exported FBO texture as DMA-BUF: fd=%d, %dx%d, stride=%d\n",
...
static void render_frame_hybrid_zero_copy(struct plugin_state *state, int frame_count) {
    ...
    if (state->dmabuf_fd >= 0) {
        ...
        if (dmabuf_fd_to_send >= 0) {
            ...
            loggy_opengl_desktop2( "[PLUGIN] ✅ Sent and closed dup'd DMA-BUF (orig fd=%d)\n", state->dmabuf_fd);

        } else {
             loggy_opengl_desktop2( "[PLUGIN] ❌ dup() failed for sending frame: %s\n", strerror(errno));
        }
    } else {
        loggy_opengl_desktop2( "[PLUGIN] No DMA-BUF available, falling back to SHM\n");
        // SHM fallback
    }
}
...
static void render_with_pool_smart(struct plugin_state *state, int frame_count) {
    ...
    if (state->dmabuf_fd >= 0) {
        ...
        if (++zero_copy_log <= 5) {
            loggy_opengl_desktop2( "[PLUGIN] ✅ TRUE ZERO-COPY render to DMA-BUF fd=%d (no glReadPixels!)\n", 
                    state->dmabuf_fd);
        }
    } else {
        ...
        if (++shm_log <= 5) {
            loggy_opengl_desktop2( "[PLUGIN] SHM fallback render (with glReadPixels)\n");
        }
    }
    ...
    if (state->content_changed_this_frame) {
        ...
        if (frame_count <= 5) {
            loggy_opengl_desktop2( "[PLUGIN] 📢 Sent notification for content change\n");
        }
    }
...
static bool init_fbo(struct plugin_state *state, int width, int height) {
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        loggy_opengl_desktop2( "Framebuffer is not complete!\n");
        return false;
    }
...
static bool init_opengl_resources(struct plugin_state *state) {
    loggy_opengl_desktop2( "[WM] Initializing OpenGL resources\n");
    ...
    if (!linked) {
        ...
        loggy_opengl_desktop2( "Program link error: %s\n", log);
    ...
    loggy_opengl_desktop2( "[WM] OpenGL resources initialized successfully\n");
    return true;
}
...
static bool validate_window_texture(struct plugin_state *state, int window_index) {
    ...
    if (!glIsTexture(texture)) {
        loggy_opengl_desktop2( "[PLUGIN] Window %d texture %u is no longer valid\n", window_index, texture);
    ...
    if (error != GL_NO_ERROR) {
        loggy_opengl_desktop2( "[PLUGIN] Error validating texture %u: 0x%x\n", texture, error);
        state->windows[window_index].gl_texture = 0;
        state->windows[window_index].is_valid = false;
        return false;
    }
    ...
    if (width <= 0 || height <= 0) {
        loggy_opengl_desktop2( "[PLUGIN] Window %d texture has invalid dimensions: %dx%d\n", 
...
static void render_window(struct plugin_state *state, int index) {
    ...
    GLenum error = glGetError();
    if (error != GL_NO_ERROR) {
        loggy_opengl_desktop2( "[PLUGIN] Error binding texture %u: 0x%x\n", 
...
static bool check_framebuffer_status(struct plugin_state *state) {
    if (!state->fbo) {
        loggy_opengl_desktop2( "[WM] Warning: No FBO to check\n");
        return false;
    }
    ...
    switch (status) {
        ...
        case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
            loggy_opengl_desktop2( "[WM] Error: Framebuffer incomplete - attachment issue\n");
            break;
            
        case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
            loggy_opengl_desktop2( "[WM] Error: Framebuffer incomplete - missing attachment\n");
            break;
            
        case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS:
            loggy_opengl_desktop2( "[WM] Error: Framebuffer incomplete - dimension mismatch\n");
            break;
            
        case GL_FRAMEBUFFER_UNSUPPORTED:
            loggy_opengl_desktop2( "[WM] Error: Framebuffer unsupported format\n");
            break;
            
        default:
            loggy_opengl_desktop2( "[WM] Error: Unknown framebuffer status: 0x%x\n", status);
            break;
    }
...
static bool init_dmabuf_pool(struct plugin_state *state) {
    loggy_opengl_desktop2( "[PLUGIN] === STEP 1: Creating DMA-BUF pool ===\n");
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (fd < 0) {
            loggy_opengl_desktop2( "[PLUGIN] ❌ Failed to create pool DMA-BUF %d\n", i);
            return false;
        }
        ...
        loggy_opengl_desktop2( "[PLUGIN] ✅ Created pool buffer %d: fd=%d\n", i, fd);
    }
    
    state->dmabuf_pool.initialized = true;
    loggy_opengl_desktop2( "[PLUGIN] ✅ Pool ready with %d buffers\n", 4);
    return true;
}
...
static bool init_dmabuf_fbo(struct plugin_state *state, int width, int height) {
    loggy_opengl_desktop2( "[PLUGIN] Attempting to create DMA-BUF backed FBO (%dx%d)\n", width, height);
    ...
    state->dmabuf_fd = create_dmabuf(width * height * 4, state->instance_id);
    if (state->dmabuf_fd < 0) {
        loggy_opengl_desktop2( "[PLUGIN] Failed to create DMA-BUF\n");
        return false;
    }
    loggy_opengl_desktop2( "[PLUGIN] Created DMA-BUF fd: %d\n", state->dmabuf_fd);
    ...
    if (!eglCreateImageKHR) {
        loggy_opengl_desktop2( "[PLUGIN] Missing eglCreateImageKHR extension\n");
        close(state->dmabuf_fd);
        state->dmabuf_fd = -1;
        return false;
    }
    ...
    if (!glEGLImageTargetTexture2DOES) {
        loggy_opengl_desktop2( "[PLUGIN] Missing glEGLImageTargetTexture2DOES extension\n");
        close(state->dmabuf_fd);
        state->dmabuf_fd = -1;
        return false;
    }
    
    loggy_opengl_desktop2( "[PLUGIN] EGL extensions available\n");
    ...
    loggy_opengl_desktop2( "[PLUGIN] Creating EGL image with format ABGR8888, pitch %d\n", width * 4);
    
    state->egl_image = eglCreateImageKHR(state->egl_display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, NULL, attribs);
    if (state->egl_image == EGL_NO_IMAGE_KHR) {
        EGLint error = eglGetError();
        loggy_opengl_desktop2( "[PLUGIN] Failed to create EGL image: 0x%x\n", error);
        close(state->dmabuf_fd);
        state->dmabuf_fd = -1;
        return false;
    }
    
    loggy_opengl_desktop2( "[PLUGIN] Created EGL image successfully\n");
    ...
    GLenum fb_status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
    if (fb_status != GL_FRAMEBUFFER_COMPLETE) {
        loggy_opengl_desktop2( "[PLUGIN] DMA-BUF framebuffer not complete: 0x%x\n", fb_status);
        glDeleteFramebuffers(1, &state->fbo);
        glDeleteTextures(1, &state->fbo_texture);
        // Clean up EGL image
        PFNEGLDESTROYIMAGEKHRPROC eglDestroyImageKHR = (PFNEGLDESTROYIMAGEKHRPROC)eglGetProcAddress("eglDestroyImageKHR");
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    loggy_opengl_desktop2( "[PLUGIN] ✅ Successfully created DMA-BUF backed FBO (%dx%d)\n", width, height);
    return true;
}
...
static bool send_dmabuf_info_once(struct plugin_state *state) {
   // CRITICAL FIX: Only send the DMA-BUF info TRULY ONCE
    ...
    loggy_opengl_desktop2( "[PLUGIN] Sending DMA-BUF info for the FIRST AND ONLY TIME (fd: %d)\n", state->dmabuf_fd);
    ...
    if (fd_to_send < 0) {
        loggy_opengl_desktop2( "[PLUGIN] Failed to dup DMA-BUF fd: %s\n", strerror(errno));
        return false;
    }
    ...
    *((int*)CMSG_DATA(cmsg)) = fd_to_send;
    ...
    if (sendmsg(state->ipc_fd, &msg, 0) < 0) {
        loggy_opengl_desktop2( "[PLUGIN] Failed to send DMA-BUF info: %s\n", strerror(errno));
    ...
    loggy_opengl_desktop2( "[PLUGIN] ✅ DMA-BUF info sent ONCE - will never send again\n");
    return true;
}
...
static void send_frame_ready(struct plugin_state *state) {
    ...
    if (write(state->ipc_fd, &notification, sizeof(notification)) < 0) {
        loggy_opengl_desktop2( "[PLUGIN] Failed to send frame ready: %s\n", strerror(errno));
        return;
    }
    ...
    if (++log_count <= 5) {
        loggy_opengl_desktop2( "[PLUGIN] ✅ Frame ready notification sent (seq=%u, NO FDs)\n", 
                state->last_frame_sequence);
    }
}
...
static void render_desktop_to_fbo(struct plugin_state *state) {
   loggy_opengl_desktop2("[PLUGIN] Starting render_desktop_to_fbo\n");  // Only prints first 5 times
    loggy_opengl_desktop2("[PLUGIN] Window count: %d\n", state->window_count);
    
    if (!check_framebuffer_status(state)) {
        loggy_opengl_desktop2("[PLUGIN] Framebuffer not complete, skipping render\n");
        return;
    }
    ...
    loggy_opengl_desktop2("[PLUGIN] render_desktop_to_fbo complete\n");
}
...
ssize_t n = recvmsg(sock_fd, &msg, 0);
    if (n <= 0) {
        if (n < 0) {
            loggy_opengl_desktop2( "[PLUGIN] ❌ recvmsg FAILED: %s\n", strerror(errno));
        }
        return n;
    }
    ...
    *fd_count = 0;
    ...
    loggy_opengl_desktop2( "[PLUGIN] recvmsg received %zd bytes\n", n);
    
    struct cmsghdr *cmsg = CMSG_FIRSTHDR(&msg);
    if (!cmsg) {
        loggy_opengl_desktop2( "[PLUGIN] No control message (no FDs sent)\n");
    }
    
    while (cmsg != NULL) {
        loggy_opengl_desktop2( "[PLUGIN] Control message: level=%d, type=%d, len=%zu\n", 
        ...
        if (cmsg->cmsg_level == SOL_SOCKET && cmsg->cmsg_type == SCM_RIGHTS) {
            ...
            loggy_opengl_desktop2( "[PLUGIN] ✅ Received %d FDs: ", num_fds_in_msg);
            for (int i = 0; i < num_fds_in_msg; i++) {
                if (*fd_count < 32) {
                    fds[*fd_count] = received_fds[i];
                    loggy_opengl_desktop2( "%d ", received_fds[i]);
                    (*fd_count)++;
                } else {
                    loggy_opengl_desktop2( "(overflow, closing %d) ", received_fds[i]);
                    close(received_fds[i]);
                }
            }
            loggy_opengl_desktop2( "\n");
        }
...
static GLuint texture_from_shm(struct plugin_state *state, const ipc_window_info_t *info) {
    if (info->fd < 0) {
        loggy_opengl_desktop2( "[WM] Invalid SHM file descriptor.\n");
        return 0;
    }
    ...
    GLenum gl_error = glGetError();
    if (gl_error != GL_NO_ERROR) {
        loggy_opengl_desktop2( "[WM] OpenGL error creating texture from SHM: 0x%x\n", gl_error);
        glDeleteTextures(1, &texture_id);
        return 0;
    }

    loggy_opengl_desktop2( "[WM] Successfully created texture %u from SHM (fd:%d, %dx%d)\n", 
...
static GLuint create_texture_hybrid(struct plugin_state *state, const ipc_window_info_t *info, int window_index) {
    if (info->fd < 0) {
        loggy_opengl_desktop2( "[PLUGIN] Invalid file descriptor for window %d\n", window_index);
        return 0;
    }
    ...
    if (format_log_count < 10) {
        loggy_opengl_desktop2( "[PLUGIN] Window %d format: 0x%x (%dx%d, stride=%u, modifier=0x%lx)\n", 
   ...
   GLuint texture = 0;
    ...
    loggy_opengl_desktop2( "[PLUGIN] ❌ All texture creation methods failed for window %d (fd %d)\n", window_index, info->fd);
...
static GLuint create_texture_from_dmabuf_manual(struct plugin_state *state, const ipc_window_info_t *info) {
    ...
    if (fstat(info->fd, &fd_stat) != 0) {
        loggy_opengl_desktop2( "[PLUGIN] Invalid FD %d: %s\n", info->fd, strerror(errno));
        return 0;
    }
    ...
    switch (info->format) {
        ...
        case 0x34324258: // DRM_FORMAT_XBGR8888
            ...
            break;
        default:
            loggy_opengl_desktop2( "[PLUGIN] Unsupported DMA-BUF format: 0x%x\n", info->format);
            return 0;
    }
    
    // FIXED: Ensure context is current before EGL operations
    if (!eglMakeCurrent(state->egl_display, state->egl_surface, state->egl_surface, state->egl_context)) {
        loggy_opengl_desktop2( "[PLUGIN] Failed to make EGL context current\n");
        return 0;
    }
    ...
    GLenum gl_error = glGetError();
    if (gl_error != GL_NO_ERROR) {
        loggy_opengl_desktop2( "[PLUGIN] OpenGL error creating texture from EGL image: 0x%x\n", gl_error);
...
static GLuint create_texture_with_stride_fix_optimized(struct plugin_state *state, const ipc_window_info_t *info) {
    ...
    if (info->stride != expected_stride) {
        loggy_opengl_desktop2( "[PLUGIN] Format 0x%x: stride %u vs expected %u (bpp=%u)\n", 
    ...
    void *mapped_data = mmap(NULL, map_size, PROT_READ, MAP_PRIVATE, info->fd, 0);
    if (mapped_data == MAP_FAILED) {
        loggy_opengl_desktop2( "[PLUGIN] mmap failed: %s\n", strerror(errno));
        return 0;
    }
 ...
 /*   
    ...
    GLenum error = glGetError();
    if (error != GL_NO_ERROR) {
        loggy_opengl_desktop2( "[PLUGIN] OpenGL error creating texture: 0x%x\n", error);
        glDeleteTextures(1, &texture);
        return 0;
    }
   */ 
...
static void handle_window_list_hybrid(struct plugin_state *state, ipc_event_t *event, int *fds, int fd_count) {
    ...
    if (fd_count > 0) {
        loggy_opengl_desktop2( "[PLUGIN] 🔍 RECEIVED FDs: ");
        for (int i = 0; i < fd_count; i++) {
            loggy_opengl_desktop2( "%d ", fds[i]);
        }
        loggy_opengl_desktop2( "\n");
    }
    
    loggy_opengl_desktop2( "[PLUGIN] 🔍 Processing window list: %d windows, %d FDs\n", new_window_count, fd_count);
    
    // 1. Mark all existing windows as potentially stale.
    // We will un-mark them as we process the new list.
    for (int i = 0; i < state->window_count; i++) {
        loggy_opengl_desktop2( "[PLUGIN] 🔍 Marking window %d as potentially stale (was valid=%d, texture=%u)\n", 
    ...
    for (int i = 0; i < new_window_count && i < 10; i++) {
        ...
        if (i < fd_count) {
            info->fd = fds[i];
            loggy_opengl_desktop2( "[PLUGIN] 🔍 Window %d: Assigned FD %d\n", i, info->fd);
        } else {
            info->fd = -1;
            loggy_opengl_desktop2( "[PLUGIN] 🔍 Window %d: No FD available\n", i);
        }
        ...
        //loggy_opengl_desktop2( "[PLUGIN] 🔍 Window %d: Updated position/size: %.0fx%.0f at (%.0f,%.0f)\n", 
          //      i, info->width, info->height, info->x, info->y);
        
        // 3. CRITICAL CHANGE: Only update the texture if a new FD is provided.
        if (info->fd >= 0) {
            loggy_opengl_desktop2( "[PLUGIN] 🔍 Window %d: Creating new texture from FD %d\n", i, info->fd);
            ...
            if (state->windows[i].gl_texture != 0) {
                loggy_opengl_desktop2( "[PLUGIN] 🔍 Window %d: Deleting old texture %u\n", i, state->windows[i].gl_texture);
                glDeleteTextures(1, &state->windows[i].gl_texture);
                state->windows[i].gl_texture = 0;
            }
            
            // Create the new texture from the provided FD.
            loggy_opengl_desktop2( "[PLUGIN] 🔍 Window %d: Calling create_texture_hybrid with FD %d\n", i, info->fd);
            GLuint new_texture = create_texture_hybrid(state, info, i);
            loggy_opengl_desktop2( "[PLUGIN] 🔍 Window %d: create_texture_hybrid returned texture %u\n", i, new_texture);
            
            if (new_texture > 0) {
                state->windows[i].gl_texture = new_texture;
                state->content_changed_this_frame = true; // Signal that a re-render is needed.
                loggy_opengl_desktop2( "[PLUGIN] ✅ Recreated texture %u for window %d (fd=%d)\n", new_texture, i, info->fd);
            } else {
                // Creation failed. Mark the window as invalid so we don't try to render it.
                state->windows[i].is_valid = false;
                loggy_opengl_desktop2( "[PLUGIN] ❌ Failed to create texture for window %d (fd=%d)\n", i, info->fd);
            }
            
            // We are done with this FD, so close it to prevent leaks.
            loggy_opengl_desktop2( "[PLUGIN] 🔍 Window %d: About to close FD %d\n", i, info->fd);
            if (close(info->fd) == 0) {
                loggy_opengl_desktop2( "[PLUGIN] ✅ Window %d: Successfully closed FD %d\n", i, info->fd);
            } else {
                loggy_opengl_desktop2( "[PLUGIN] ❌ Window %d: Failed to close FD %d: %s\n", i, info->fd, strerror(errno));
            }
            info->fd = -1;
        } else {
            loggy_opengl_desktop2( "[PLUGIN] 🔍 Window %d: No new FD, keeping existing texture %u\n", i, state->windows[i].gl_texture);
        }
        
        // If the existing texture is 0, the window is invalid until a new buffer is provided.
        if (state->windows[i].gl_texture == 0) {
            loggy_opengl_desktop2( "[PLUGIN] ⚠️  Window %d: No texture available, marking invalid\n", i);
            state->windows[i].is_valid = false;
        } else {
            loggy_opengl_desktop2( "[PLUGIN] ✅ Window %d: Valid with texture %u\n", i, state->windows[i].gl_texture);
        }
    }
    
    // 4. Clean up textures for windows that no longer exist.
    if (new_window_count < state->window_count) {
        loggy_opengl_desktop2( "[PLUGIN] 🔍 Cleaning up %d removed windows\n", state->window_count - new_window_count);
        for (int i = new_window_count; i < state->window_count; i++) {
            if (state->windows[i].gl_texture != 0) {
                loggy_opengl_desktop2( "[PLUGIN] 🔍 Removing window %d texture %u\n", i, state->windows[i].gl_texture);
                glDeleteTextures(1, &state->windows[i].gl_texture);
                state->windows[i].gl_texture = 0;
            }
        }
    ...
    if (fd_count > new_window_count) {
        loggy_opengl_desktop2( "[PLUGIN] 🔍 Closing %d unclaimed FDs\n", fd_count - new_window_count);
        for (int i = new_window_count; i < fd_count; i++) {
            if (fds[i] >= 0) {
                loggy_opengl_desktop2( "[PLUGIN] 🔍 Closing unclaimed FD %d\n", fds[i]);
                if (close(fds[i]) == 0) {
                    loggy_opengl_desktop2( "[PLUGIN] ✅ Successfully closed unclaimed FD %d\n", fds[i]);
                } else {
                    loggy_opengl_desktop2( "[PLUGIN] ❌ Failed to close unclaimed FD %d: %s\n", fds[i], strerror(errno));
                }
            }
        }
    }
    ...
    loggy_opengl_desktop2( "[PLUGIN] 🔍 Updating window count: %d -> %d\n", state->window_count, new_window_count);
    state->window_count = new_window_count;
    
    // Debug: Final window state summary
    loggy_opengl_desktop2( "[PLUGIN] 🔍 FINAL STATE: %d windows\n", state->window_count);
    for (int i = 0; i < state->window_count; i++) {
        loggy_opengl_desktop2( "[PLUGIN] 🔍   Window %d: valid=%d, texture=%u\n", 
                i, state->windows[i].is_valid, state->windows[i].gl_texture);
    }
    ...
    if (++check_counter % 10 == 0) {  // Every 10 calls
        ...
        loggy_opengl_desktop2( "[PLUGIN] 📊 Open FDs: %d/1024 (check #%d)\n", open_fds, check_counter);
        
        if (open_fds > 100) {
            loggy_opengl_desktop2( "[PLUGIN] ⚠️  HIGH FD USAGE DETECTED!\n");
        }
    }
}
...
static int export_dmabuf_fallback(struct plugin_state *state) {
    if (state->dmabuf_fd < 0) {
        loggy_opengl_desktop2( "[PLUGIN] No DMA-BUF FD available for fallback export\n");
        return -1;
    }
    ...
    loggy_opengl_desktop2( "[PLUGIN] ✅ Exported DMA-BUF via fallback: fd=%d\n", export_fd);
    return export_fd;
}
...
static void drain_ipc_queue(struct plugin_state *state) {
    ...
    if (drained >= 10) {
        loggy_opengl_desktop2("[PLUGIN] Drained max messages, more may be pending\n");
    }
}
...
static void render_with_proper_sync_FIXED(struct plugin_state *state, int frame_count) {
    ...
    if (frame_count <= 10 || frame_count % 300 == 0) {
        loggy_opengl_desktop2( "[PLUGIN] Frame %d: rendered %d windows\n", frame_count, rendered_count);
    }
}
...
int main(int argc, char *argv[]) {
    loggy_opengl_desktop2( "[PLUGIN] === HYBRID FAST VERSION WITH DMA-BUF OUTPUT ===\n");
    ...
    if (ipc_fd < 0 || !shm_name) {
        loggy_opengl_desktop2( "[PLUGIN] ERROR: Invalid arguments\n");
        return 1;
    }

    loggy_opengl_desktop2( "[PLUGIN] Starting hybrid plugin with DMA-BUF output\n");
    ...
    loggy_opengl_desktop2( "[%s] Starting isolated plugin instance\n", state.instance_name);
...
state.dmabuf_sent_to_compositor = false;
    ...
    if (shm_fd < 0) { 
        loggy_opengl_desktop2( "[PLUGIN] ERROR: shm_open failed: %s\n", strerror(errno));
        return 1; 
    }
    
    state.shm_ptr = mmap(NULL, SHM_BUFFER_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
    if (state.shm_ptr == MAP_FAILED) { 
        loggy_opengl_desktop2( "[PLUGIN] ERROR: mmap failed: %s\n", strerror(errno));
        close(shm_fd); 
        return 1; 
    }
    close(shm_fd);
    loggy_opengl_desktop2( "[PLUGIN] ✅ SHM buffer ready (fallback)\n");

    // Initialize EGL context
    loggy_opengl_desktop2( "[PLUGIN] Initializing EGL context...\n");
    if (!init_plugin_egl(&state)) {
        loggy_opengl_desktop2( "[PLUGIN] ERROR: EGL initialization failed\n");
        return 1;
    }
    loggy_opengl_desktop2( "[PLUGIN] ✅ EGL context ready\n");

// Right after: loggy_opengl_desktop2( "[PLUGIN] ✅ EGL context ready\n");

// CRITICAL: Initialize EGL extensions
loggy_opengl_desktop2( "[PLUGIN] Initializing EGL extensions...\n");
if (!init_egl_extensions(state.egl_display)) {
    loggy_opengl_desktop2( "[PLUGIN] ⚠️  EGL DMA-BUF export extensions not available\n");
    loggy_opengl_desktop2( "[PLUGIN] Will use fallback DMA-BUF sharing\n");
    // Set flag to use fallback method
    state.use_egl_export = false;
} else {
    loggy_opengl_desktop2( "[PLUGIN] ✅ EGL DMA-BUF export extensions ready\n");
    state.use_egl_export = true;
}

    // Initialize OpenGL resources
    loggy_opengl_desktop2( "[PLUGIN] Initializing OpenGL resources...\n");
    if (!init_opengl_resources(&state)) {
        loggy_opengl_desktop2( "[PLUGIN] ERROR: OpenGL initialization failed\n");
        return 1;
    }
    loggy_opengl_desktop2( "[PLUGIN] ✅ OpenGL resources ready\n");

    // STEP 1: Test pool creation
loggy_opengl_desktop2( "[PLUGIN] Testing DMA-BUF pool creation...\n");
if (init_dmabuf_pool(&state)) {
    loggy_opengl_desktop2( "[PLUGIN] ✅ Pool creation successful!\n");
} else {
    loggy_opengl_desktop2( "[PLUGIN] ❌ Pool creation failed\n");
}
...
// Add this right after: loggy_opengl_desktop2( "[PLUGIN] ✅ Pool rendering test complete\n");

// STEP 4: Send pool to compositor
if (send_pool_to_compositor(&state)) {
    loggy_opengl_desktop2( "[PLUGIN] ✅ Pool transfer successful!\n");
} else {
    loggy_opengl_desktop2( "[PLUGIN] ❌ Pool transfer failed!\n");
}

    // Try to initialize DMA-BUF backed FBO (the key change!)
    loggy_opengl_desktop2( "[PLUGIN] Attempting DMA-BUF backed FBO...\n");
...
bool dmabuf_fbo_available = init_dmabuf_fbo_robust(&state, SHM_WIDTH, SHM_HEIGHT);
    ...
    if (!dmabuf_fbo_available) {
        loggy_opengl_desktop2( "[PLUGIN] DMA-BUF FBO failed, falling back to regular FBO...\n");
        if (!init_fbo(&state, SHM_WIDTH, SHM_HEIGHT)) {
            loggy_opengl_desktop2( "[PLUGIN] ERROR: Fallback FBO initialization failed\n");
            return 1;
        }
    }

    // Try to initialize shared DMA-BUF context for window import
    loggy_opengl_desktop2( "[PLUGIN] Attempting shared DMA-BUF context...\n");
    bool dmabuf_lib_available = dmabuf_share_init_with_display(&state.dmabuf_ctx, state.egl_display);
    if (dmabuf_lib_available) {
        loggy_opengl_desktop2( "[PLUGIN] ✅ Shared DMA-BUF library available\n");
    } else {
        loggy_opengl_desktop2( "[PLUGIN] ⚠️  Shared DMA-BUF library not available\n");
    }

    loggy_opengl_desktop2( "[PLUGIN] 🚀 Hybrid plugin ready!\n");
    loggy_opengl_desktop2( "[PLUGIN] Capabilities:\n");
    loggy_opengl_desktop2( "[PLUGIN]   - DMA-BUF output FBO: %s\n", dmabuf_fbo_available ? "YES" : "NO");
    loggy_opengl_desktop2( "[PLUGIN]   - Shared DMA-BUF library: %s\n", dmabuf_lib_available ? "YES" : "NO");
    loggy_opengl_desktop2( "[PLUGIN]   - EGL DMA-BUF import: YES\n");
    loggy_opengl_desktop2( "[PLUGIN]   - SHM fallback: YES\n");
    ...
    loggy_opengl_desktop2( "[PLUGIN] ✅ IPC connection verified\n");

    // Send DMA-BUF info once if available
    if (dmabuf_fbo_available) {
        if (send_dmabuf_info_once(&state)) {
            loggy_opengl_desktop2( "[PLUGIN] ✅ DMA-BUF output enabled - zero-copy to compositor!\n");
          //  verify_dmabuf_zero_copy(&state);
        } else {
            loggy_opengl_desktop2( "[PLUGIN] ⚠️  DMA-BUF output failed, will use SHM fallback\n");
            dmabuf_fbo_available = false;
        }
    }

    loggy_opengl_desktop2( "[PLUGIN] Entering main event loop...\n");
...
if (select_result == 0) {
    ...
    if (frame_count <= 5) {
      ...
      //  glClear(GL_COLOR_BUFFER_BIT);
        ...
        loggy_opengl_desktop2( "[PLUGIN] Test render complete (color %d)\n", color_idx);
    }
  ...
  // Handle IPC events with enhanced error checking
        ...
        if (n <= 0) {
            if (n == 0) {
                loggy_opengl_desktop2( "[PLUGIN] IPC connection closed by compositor\n");
            } else {
                loggy_opengl_desktop2( "[PLUGIN] IPC read error: %s\n", strerror(errno));
            }
            break;
        }
        ...
        // Validate event type
        if (event.type != IPC_EVENT_TYPE_WINDOW_LIST_UPDATE) {
            loggy_opengl_desktop2( "[PLUGIN] Unknown IPC event type: %d\n", event.type);
            continue;
        }
    ...
    // Cleanup
    loggy_opengl_desktop2( "[PLUGIN] Cleaning up hybrid plugin...\n");
    ...
    loggy_opengl_desktop2( "[PLUGIN] Hybrid plugin shutdown complete\n");
    return 0;
}
